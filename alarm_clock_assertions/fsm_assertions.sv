...
//                                                                                                              next_state should be SHOW_TIME itself
        property show_time_st_check;
            ...
            disable iff (reset)
            (pre_state == SHOW_TIME)
             |-> ((alarm_button && next_state == SHOW_ALARM) ||
             (!alarm_button && key != NOKEY && next_state == KEY_STORED) ||
             (!alarm_button && key == NOKEY && next_state == SHOW_TIME));
        endproperty
...
//                                                                                       next_state should be KEY_WAITED itself
        ...
        |-> ((key == NOKEY && next_state == KEY_ENTRY) ||
             (key != NOKEY && time_out == 0 && next_state == SHOW_TIME) ||
             (key != NOKEY && time_out == 1 && next_state == KEY_WAITED));
        endproperty
...
//                                                                                     next_state should be KEY_ENTRY itself
    ...
    |-> ((alarm_button && next_state == SET_ALARM_TIME) ||
         (!alarm_button && time_button && next_state == SET_CURRENT_TIME) ||
         (!alarm_button && !time_button && time_out == 1'b0 &&
          next_state == SHOW_TIME) ||
         (!alarm_button && !time_button && time_out == 1'b1 &&
          key != NOKEY && next_state == KEY_STORED) ||
         (!alarm_button && !time_button && time_out == 1'b1 &&
          key == NOKEY && next_state == KEY_ENTRY));
endproperty
...
//                                                                                       next_state should be SHOW_ALARM itself
        property show_alarm_st_check;
          @(posedge clock)
        (pre_state == SHOW_ALARM)
        |-> ((!alarm_button && next_state == SHOW_TIME) ||
             (alarm_button && next_state == SHOW_ALARM));
        endproperty
...
TIME_OUT_CHECK            : assert property (time_out_check);
