...
// RESET_CHECK
// On reset, current time should be 00:00
...
// TIMER_WRAP_BACK_CHECK
// If one_minute is high and current time is 23:59,
// next current time should be 00:00
...
// LOAD_NEW_CURRENT_TIME_CHECK
// If load_new_c is high, new_current_time and current_time
// should be same in the next cycle
...
property load_new_current_time;
        @(posedge clk)
        hour_seq3 |=>
        ((current_time_ms_hr  == $past(new_current_time_ms_hr)) &&
         (current_time_ls_hr  == $past(new_current_time_ls_hr)) &&
         (current_time_ms_min == $past(new_current_time_ms_min)) &&
         (current_time_ls_min == $past(new_current_time_ls_min)));
endproperty


// LS_HR_WRAP_BACK_CHECK
// If one_minute is high and current time is 09:59,
// next current time should be 10:00
...
sequence hour_seq6;
        ((current_time_ms_hr  == ($past(current_time_ms_hr) + 1'b1)) &&
         (current_time_ls_hr  == 4'd0) &&
         (current_time_ms_min == 4'd0) &&
         (current_time_ls_min == 4'd0));
endsequence
...
// MIN_WRAP_BACK_CHECK
// If one_minute is high and current time is 00:59,
// minute digits should become zero
...
// LS_MIN_INCR_CHECK
// If one_minute is high and current_time_ls_min is less than 9,
// current_time_ls_min should increment by 1
...
property minute_incr_check;
        @(posedge clk)
        min_seq4 |=>
        (current_time_ls_min == ($past(current_time_ls_min) + 1'b1));
endproperty
